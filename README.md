...
我们提供了一个强大的显存分析工具 `analyze_memory.py`，它可以：

1.  **自动化分析**：从 `profiler_logs` 目录中读取显存事件日志。
2.  **生成报告**：输出详细的Markdown格式报告，包括峰值显存、显存消耗曲线和潜在优化点。
3.  **可视化**：绘制显存使用情况图表，直观展示显存变化。

#### 报告解读

-   **峰值显存占用 (Peak Memory Usage)**：整个推理过程中达到的最大显存值。
-   **显存曲线 (Memory Curve)**：展示了显存随时间变化的趋势，帮助定位显存激增的环节。
-   **算子级分析 (Operator-level Analysis)**：列出最消耗显存的Top 10算子，为优化提供精确目标。
...
1.  **执行测试**
    ...
    使用以下脚本来运行内存测试。脚本会自动处理环境激活、模型推理和日志记录。请将 `<path_to_config_file>` 替换为实际的配置文件路径，例如 `tests/configs/baseline.yaml`。
    ...
    # 执行测试脚本
    bash tests/scripts/run_memory_tests.sh <path_to_config_file>
    ```

2.  **数据分析**

    测试脚本执行完毕后，会自动调用分析工具生成一份详细的 Markdown 报告。报告将保存在项目根目录下，文件名类似于 `enhanced_memory_analysis_report_YYYYMMDD_HHMMSS.md`。

    如果需要手动分析，可以使用以下命令：

    ```bash
    python3 tests/analyzer.py --log_file profiler_logs/baseline/memory_usage.log --output_file custom_report.md
    ```
